...
=======
  options(app.getOptions()), qtApp(qtApp), view(new View(valueSet)),amdialog(this),sscdialog(this){
>>>>>>> xutian push
...
=======
  //amDialog
  connect(ui->actionAlgorithm_Mode,SIGNAL(triggered()),this,SLOT(on_actionAlgorithm_Mode_clicked()));
  // //sscDialog  
 
  connect(ui->actionShowCorner,SIGNAL(triggered()),this,SLOT(on_actionShowCorner_triggered(true)));
  connect(&sscdialog,SIGNAL(SendData(double)),this,SLOT(on_sscdConnect(double)));
  // ConcurrentTaskManager
  taskMan.addObserver(this);
  taskCompleteEvent = QEvent::registerEventType();
>>>>>>> xutian push
...
void QtWin::init() {
  // Start animation timer启动动画计时器
  animationTimer.setSingleShot(false);
  connect(&animationTimer, SIGNAL(timeout()), this, SLOT(animate()));
  animationTimer.start(50);

  // Simulation and Tool View tabs are not closeable模拟和工具视图选项卡不可关闭
<<<<<<< HEAD
  ui->fileTabManager->setTabsClosable(true);
   ui->coordtabmanager->setTabsClosable(true);
...
>>>>>>> xutian push
  ...
  // Hide console by default默认情况下隐藏控制台
  hideConsole();
...
<<<<<<< HEAD
=======
    // ui->coordtabmanager->setCurrentIndex(0);
>>>>>>> xutian push
  updateActions();
  ...
  // Update fullscreen menu item更新全屏菜单项
...
=======
  ...
  // ui->fileTabManager->closeAll(false, true);
  // ui->fileTabManager->setCurrentIndex(0);
    ui->coordtabmanager->closeAll(false, true);
    ui->coordtabmanager->setCurrentIndex(0);
    ui->coordtabmanager->closeAll(false, true);
    ui->coordtabmanager->setCurrentIndex(0);
>>>>>>> xutian push
...
void QtWin::openProject(const string &_filename) {
  ...
  try {
    ...
    if (!is_project) {
      ...
      // Check if .camotics files exists检查一下。camotics文件存在
    ...
    if (is_project) project = new Project::Project(filename);
      ...
      // Otherwise, create a new project with the file否则，请使用该文件创建一个新项目
      if (!runNewProjectDialog()) return;
      ...
      // Save tool table before resetting project重置项目前保存工具表
      GCode::ToolTable toolTable = getNewToolTable();
      GCode::Units units = getNewUnits();
...
<<<<<<< HEAD
=======
  // if (!ui->coordtabmanager->checkSaveAll()) return false;
>>>>>>> xutian push
...
void QtWin::activateFile(const string &filename, int line, int col) {
  ...
  if (!file.isNull()) {
    // ui->fileTabManager->open(file, line, col);
    ui->coordtabmanager->open(file, line, col);
  }
}
...
<<<<<<< HEAD
=======
  //   unsigned tab = ui->coordtabmanager->currentIndex();
  // bool fileTab = ui->coordtabmanager->isFileTab(tab);
>>>>>>> xutian push
...
void QtWin::animate() {
  try {
    ...
    // Auto close after auto play自动播放后自动关闭
...
<<<<<<< HEAD
      ui->coordtabmanager->displayGcode(view->path->getProgramLine());

    }

    // Copy log
    // ui->coordview->writeToview();
   ui->console->writeToConsole();
  //  ui->console->writeGcode();
=======
      //
      if(!view->path->getProgramLine()){
...
=======
//sscdialong 
void QtWin::on_sscdConnect(double str){
    qDebug()<< str<<endl;
    sscdialog.hide();
}
>>>>>>> xutian push
...
void QtWin::on_actionPlay_triggered() {
  ...
  // cout << view->path->getProgramLine() << endl;
  }
...
=======
void QtWin::on_actionAlgorithm_Mode_clicked(){
  
AmDialog *amdl = new AmDialog(this);
amdl->exec();
}
>>>>>>> xutian push
void QtWin::on_actionShowCorner_triggered(bool checked) {
 
  // QMessageBox msgBox;
	if(checked)
	{
<<<<<<< HEAD
	view->path->updateSharpCorner(true);

	}
	else
	{
  view->path->updateSharpCorner(false);
