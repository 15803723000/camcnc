...
<<<<<<< HEAD
void ToolPathView::updateSharpCorner(bool showCorner) {

  if (!path.isNull()) {
    GCode::ShowSharpCorner sharpCorner(path);
    sharpCorner.computeSharpCorner();
    if (showCorner) sharpCorner.setCornerType();
    if (!showCorner) sharpCorner.deleteCornerType();
    dirty = true;
  }
  update();
}
...
<<<<<<< HEAD
  ...
  if (showIntensity && !path.isNull())
    for (auto it = path->begin(); it != path->end(); it++){
      ...
      if (maxDistance < fabs(it->getDistance())) maxDistance = fabs(it->getDistance());
      if (minDistance > fabs(it->getDistance())) minDistance = fabs(it->getDistance());
    }
      

  // Find position on path
=======
  // Find maximum speed找到最大速度
  double maxSpeed = 0;
  if (showIntensity && !path.isNull())
    for (auto it = path->begin(); it != path->end(); it++)
      if (maxSpeed < fabs(it->getSpeed())) maxSpeed = fabs(it->getSpeed());

  // Find position on path在路径上找到位置
>>>>>>> xutian push
  if (!path.isNull())
    for (auto it = path->begin(); it != path->end(); it++) {
      GCode::Move move = *it;
...
<<<<<<< HEAD
      ...
      bool partial = false;
=======
      uint32_t moveLine = move.getLine() + 1; // EMC2 counts from zeroEMC2从零开始计数
            bool partial = false;
>>>>>>> xutian push
...
=======
          // TODO should find the closest point on the closest move at this lineTODO应该在这条线上最近的移动中找到最近的点
>>>>>>> xutian push
          if (distance < 0.00001) {
...
<<<<<<< HEAD
      double s = (showIntensity && maxSpeed) ? fabs(move.getSpeed()) / maxSpeed : 1;

      //double s = moveDistance / (maxDistance - minDistance);
   
      if(!showIntensity) s = 1.0;
=======
      double s =
